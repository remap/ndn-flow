...
// TODO: use updated ndn-dot-net with BasicIdentityStorage when integrating with Unity code
...
namespace TestNdnDotNet {
  class TestPublishAsyncNfd {
    ...
    class Echo : OnInterestCallback, OnRegisterFailed {
      public Echo(KeyChain keyChain, Name certificateName, Name prefix)
      {
        keyChain_ = keyChain;
      ...
      {
        string commandVerb = interest.getName().get(prefix_.size()).toEscapedString();
        if (commandVerb == "match") {
          // TODO: command interest verification
          string matchId = interest.getName().get(prefix_.size() + 1).toEscapedString();
    ...
    static void Main(string[] args)
    {
      ...
      var prefix = new Name("/home/flow/unity1/");
      var echo = new Echo(keyChain, certificateName, prefix);
