...
/* This is a replicate of SegmentFetcherDataName inside ndn-js library but with data name reported for exclusion in ndn-flow */

/**
 * SegmentFetcherDataName is a utility class to fetch the latest version of segmented data.
 *
 * SegmentFetcherDataName assumes that the data is named /<prefix>/<version>/<segment>,
 * where:
 ...
 *
 * The following logic is implemented in SegmentFetcherDataName:
 *
 ...
 *
 *     SegmentFetcherDataName.fetch(face, interest, null, onComplete, onError);
 *
 * This is a private constructor to create a new SegmentFetcherDataName to use the Face.
 * An application should use SegmentFetcherDataName.fetch. If validatorKeyChain is not
 * null, use it and ignore verifySegment. After creating the SegmentFetcherDataName,
 ...
 * SegmentFetcherDataName.ErrorCode.SEGMENT_VERIFICATION_FAILED.
 ...
 * SegmentFetcherDataName.ErrorCode and message is a related string.
 * NOTE: The library will log any exceptions thrown by this callback, but for
 * better error handling the callback should catch and properly handle any
 * exceptions.
 * @constructor
 */
var SegmentFetcherDataName = function SegmentFetcherDataName
 ...
 */
SegmentFetcherDataName.ErrorCode = {
  INTEREST_TIMEOUT: 1,
 ...
 */
SegmentFetcherDataName.DontVerifySegment = function(data)
{
  return true;
};
 ...
 * verifySegment(data) where data is a Data object. If it returns False then
 * abort fetching and call onError with
 * SegmentFetcherDataName.ErrorCode.SEGMENT_VERIFICATION_FAILED. If data validation is
 * not required, use SegmentFetcherDataName.DontVerifySegment.
 * NOTE: The library will log any exceptions thrown by this callback, but for
 * better error handling the callback should catch and properly handle any
 * exceptions.
 * @param {function} onComplete When all segments are received, call
 ...
 * exceptions.
 */
SegmentFetcherDataName.fetch = function
  (face, baseInterest, validatorKeyChainOrVerifySegment, onComplete, onError)
{
  if (validatorKeyChainOrVerifySegment == null ||
    ...
    new SegmentFetcherDataName
      (face, validatorKeyChainOrVerifySegment, SegmentFetcherDataName.DontVerifySegment,
       onComplete, onError)
...
SegmentFetcherDataName.prototype.fetchFirstSegment = function(baseInterest)
{
  var interest = new Interest(baseInterest);
  interest.setChildSelector(1);
  interest.setMustBeFresh(true);
  var thisSegmentFetcherDataName = this;
  this.face.expressInterest
    (interest,
     function(originalInterest, data)
       { thisSegmentFetcherDataName.onData(originalInterest, data); },
     function(interest) { thisSegmentFetcherDataName.onTimeout(interest); });
};

SegmentFetcherDataName.prototype.fetchNextSegment = function
...
SegmentFetcherDataName.prototype.onData = function(originalInterest, data)
{
  if (this.validatorKeyChain != null) {
    try {
      ...
      this.validatorKeyChain.verifyData
        (data,
         function(localData) {
           thisSegmentFetcherDataName.onVerified(localData, originalInterest);
         },
...
SegmentFetcherDataName.prototype.onVerified = function(data, originalInterest)
{
  if (!SegmentFetcherDataName.endsWithSegmentNumber(data.getName())) {
    // We don't expect a name without a segment number.  Treat it as a bad packet.
    try {
      this.onError
        (SegmentFetcherDataName.ErrorCode.DATA_HAS_NO_SEGMENT,
    ...
    if (currentSegment != expectedSegmentNumber)
      ...
      if (data.getMetaInfo().getFinalBlockId().getValue().size() > 0) {
        ...
        if (currentSegment == finalSegmentNumber) {
          ...
          try {
            this.onComplete(data.getName(), new Blob(content, false));
          } catch (ex) {
...
SegmentFetcherDataName.prototype.onValidationFailed = function(data, reason)
{
  try {
    this.onError
      (SegmentFetcherDataName.ErrorCode.SEGMENT_VERIFICATION_FAILED,
...
SegmentFetcherDataName.prototype.onTimeout = function(interest)
{
  try {
    this.onError
      (SegmentFetcherDataName.ErrorCode.INTEREST_TIMEOUT,
 ...
 */
SegmentFetcherDataName.endsWithSegmentNumber = function(name)
{
